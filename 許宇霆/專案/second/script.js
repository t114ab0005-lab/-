...
function initApp() {
  ...
  const completedCount = state.tasks.filter(t => t.status === 'completed' && !t.isUserPosted).length;
  if (completedCount === 0) {
    const pastTasks = [
      {
        id: 901,
...
function createTaskCard(task, showStatusBadge = false) {
  ...
  card.className = `p-5 rounded-2xl bg-white/5 backdrop-blur-md shadow-xl border border-white/10 flex flex-col gap-3 cursor-pointer hover:-translate-y-1 hover:border-purple-500/50 transition-all ${isHighReward ? 'ring-1 ring-purple-500 shadow-[0_0_15px_rgba(139,92,246,0.2)]' : ''}`;
...
function resetQuickMatchUI() {
  ...
  document.querySelectorAll('.match-tag-btn').forEach(btn => btn.classList.remove('active', 'bg-purple-500', 'text-white'));
...
function simulateStudentSubmit(taskId) {
  ...
  renderStats();
  renderProfileTabsList();
  showToast(`模擬成功！接案學生 林育辰 已提交成果。`, 'success');
}
...
function approvePostedTask(taskId) {
  ...
  renderStats();
  renderProfileTabsList();
  showToast(`審核核准！已成功撥款 $${task.reward} 給接案學生。`, 'success');
}
...
function rejectPostedTask(taskId) {
  ...
  renderStats();
  renderProfileTabsList();
  showToast(`已退回該任務，已通知接案學生進行修改。`, 'warning');
}
